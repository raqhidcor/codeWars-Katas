...
function lastDayIsFriday(initialYear, endYear) {
    let fridays = 0;
    for (let year = initialYear; year <= (initialYear && endYear || initialYear || endYear ); year++){
      for(let month =0;month<12;month++)
        if ((new Date(year,month,0).getDay()== 5))
          fridays++
    }
    return fridays;
  }
...
//   but there is an issue with the random tests on this kata :( 